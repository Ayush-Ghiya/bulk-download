...
const SSE_ROWS: SseRow[] = [
  {
    event: "browser",
    payload: "{ message }",
    meaning: "Request leaves the browser — pacing only, no network hop.",
  },
  {
    event: "bff",
    payload: "{ message }",
    meaning: "Stands in for the dashboard BFF proxy hop.",
  },
  {
    event: "resolve",
    payload: "{ resolved, requested }",
    meaning: "catalog.findByIds ran; counts of resolved vs requested IDs.",
  },
  {
    event: "payload-write",
    payload: "{ checksum, key }",
    meaning: "payload.json written to the derived store for this checksum.",
  },
  {
    event: "sign",
    payload: "{ expiresAt }",
    meaning: "The signed download URL was minted.",
  },
  {
    event: "cdn",
    payload: "{ message }",
    meaning: "Stands in for the CDN edge hop.",
  },
  {
    event: "origin-verify",
    payload: "{ verified }",
    meaning: "Independent signer.verify() of the just-minted URL.",
  },
  {
    event: "cache-check",
    payload: "{ hit }",
    meaning: "existsDerived result — drives the HIT/MISS branch.",
  },
  {
    event: "build",
    payload: "{ name, index, total }",
    meaning: "One per entry as it is appended. MISS only.",
  },
  {
    event: "tee",
    payload: "{ message }",
    meaning:
      "Once, after the archive fully drains to client + cache. MISS only.",
  },
  {
    event: "done",
    payload: "{ downloadUrl, checksum, cacheHit, expiresAt }",
    meaning: "Terminal success. Closes the stream.",
  },
  {
    event: "error",
    payload: "{ message }",
    meaning:
      "Terminal failure — no valid assets, or a caught exception. Closes the stream.",
  },
];
    ...
    <div className="flex flex-col gap-14">
      ...
      <header className="flex flex-col gap-4 border-b border-border pb-10">
        ...
        <p className="max-w-2xl text-sm leading-relaxed text-muted-foreground">
          The demo runs as a single Bun/Hono service, but every stage mirrors
          the Media Library's real distributed &ldquo;Download All&rdquo; flow.
          This page walks the whole pipeline end to end — the content-addressed
          cache, the tee-streaming archive builder, and the HMAC-signed expiring
          links — grounded in the exact code that ships in{" "}
          <Term>server/src/</Term>.
        </p>
      </header>
      ...
      <section className="flex flex-col gap-5">
        ...
        <p className="max-w-3xl text-sm leading-relaxed text-muted-foreground">
          First the client asks the server to prepare the download over a
          streaming connection — the server resolves the selection, saves a
          record of it, and signs a link (lane A). Then the client opens that
          link as a second request, and the server re-verifies it and either
          serves a cached archive or builds one (lane B). The streaming
          connection never carries the ZIP itself — only progress updates.
        </p>
      ...
      <section className="flex flex-col gap-5">
        ...
        <p className="max-w-3xl text-sm leading-relaxed text-muted-foreground">
          The flow is two HTTP requests. First the client asks the server to
          prepare a download and gets back a signed link; then it opens that
          link to actually receive the ZIP. Here is every phase of both, in
          order.
        </p>
      ...
      <section className="flex flex-col gap-5">
        <SectionHeader
          index="03"
          label="Technique"
          title="The content-addressed, idempotent cache"
        />
        <div className="grid gap-6 lg:grid-cols-[1.05fr_1fr] lg:items-start">
          <div className="flex flex-col gap-4 text-sm leading-relaxed text-muted-foreground">
            <p>
              The whole cache turns on one idea: an archive is fully described
              by
              <Term>{"{ tenantId, zipName, entries }"}</Term>, and the SHA-256
              of that description <span className="text-foreground">is</span>{" "}
              its identity. The same selection always hashes to the same
              checksum, so the second request for a selection is a pure cache
              HIT rather than a rebuild.
            </p>
            <ul className="flex flex-col gap-2.5">
              <li>
                <span className="text-foreground">
                  Every output-affecting field is in the hash.
                </span>{" "}
                <Term>tenantId</Term> and <Term>zipName</Term> change the
                archive (scoping, the <Term>Content-Disposition</Term> filename)
                even when file bytes don't, so both are hashed. Each entry
                contributes <Term>assetId</Term>, <Term>sourceKey</Term>,{" "}
                <Term>entryName ?? null</Term>, and <Term>bytes</Term>.
              </li>
              <li>
                <span className="text-foreground">Order is significant.</span>{" "}
                Entries are hashed as an array in the caller's order (deduped
                first-occurrence by <Term>findByIds</Term>). Requesting{" "}
                <Term>[A, B]</Term> vs <Term>[B, A]</Term> yields two checksums
                and two cached archives — matching how a real ZIP's directory
                order can matter downstream.
              </li>
              <li>
                <span className="text-foreground">
                  bytes catches stale sources.
                </span>{" "}
                Captured at catalog-build time; if a source file's size changes
                on disk the next request gets a{" "}
                <span className="text-foreground">different</span> checksum and
                correctly misses the stale entry.
              </li>
            </ul>
            <p>
              That checksum then names both derived objects. Writing{" "}
              <Term>payload.json</Term> on{" "}
              <span className="text-foreground">every</span> request — hit or
              miss — is deliberate: the bytes are a pure function of the
              checksum, so re-writing is a harmless no-op, and it means the
              serve route can rebuild the <Term>BulkDownloadArchive</Term> from
              disk without the stream's in-memory state. The payload{" "}
              <span className="text-foreground">is</span> the durable record of
      ...
      <section className="flex flex-col gap-5">
        <SectionHeader
          index="04"
          label="Technique"
          title="The tee-streaming ZIP builder"
        />
        <div className="grid gap-6 lg:grid-cols-[1fr_1.05fr] lg:items-start">
          ...
          <div className="flex flex-col gap-4 text-sm leading-relaxed text-muted-foreground">
            <p>
              On a miss the ZIP is generated{" "}
              <span className="text-foreground">once</span> and piped to two
              sinks simultaneously — the client download and the derived cache.
              The first requester pays the build cost; everyone after them gets
              the cached object.
            </p>
            <ul className="flex flex-col gap-2.5">
              <li>
                <span className="text-foreground">
                  Store mode, not compression.
                </span>{" "}
                <Term>archiver("zip", {"{ store: true }"})</Term> skips
                per-entry compression — images and video are already compressed,
                so it buys little, and store mode lets the archiver behave as a
              ...
              <li>
                ...
                <Term>openSource()</Term> calls <Term>statSync()</Term> first so
                a missing file throws{" "}
                <span className="text-foreground">synchronously</span> and the
                loop can <Term>continue</Term>, instead of surfacing later as a
                stream error that tears down the whole archive.
              </li>
              <li>
                <span className="text-foreground">
                  Caching never fails the download.
                </span>{" "}
                The cache branch's <Term>error</Term> listener is a no-op; if
                opening the cache write throws, the builder just detaches that
                branch and the client stream continues. Only the client branch
                is load-bearing.
              </li>
            </ul>
            <p>
              And a partial ZIP is never mistaken for a HIT: the cache writes to{" "}
              <Term>download.zip.tmp</Term> and only <Term>rename()</Term>s to
              the real path after the stream fully finishes. Since{" "}
              <Term>existsDerived()</Term> just <Term>stat()</Term>s the final
              path — and same-filesystem <Term>rename</Term> is atomic — a
              reader either sees a complete archive or nothing at all.
            </p>
            <CodeBlock
      ...
      <section className="flex flex-col gap-5">
        <SectionHeader
          index="05"
          label="Technique"
          title="HMAC-signed, expiring links"
        />
        <div className="grid gap-6 lg:grid-cols-[1.05fr_1fr] lg:items-start">
          <div className="flex flex-col gap-4 text-sm leading-relaxed text-muted-foreground">
            <p>
              The archive endpoint serves whatever is in the derived store, so
              it needs authorization and expiry. The scheme is deliberately
              minimal:
            </p>
            ...
            <ul className="flex flex-col gap-2.5">
              <li>
                <span className="text-foreground">
                  The percent-encoded pathname is signed
                </span>{" "}
                — <Term>encodeURI(/assets/{"{tenantId}"}/download-all/…)</Term>{" "}
                — so a <Term>zipName</Term> with spaces is signed over exactly
                the bytes that travel on the wire. <Term>verify()</Term>{" "}
                round-trips through <Term>decodeURI</Term> then{" "}
                <Term>encodeURI</Term> so equivalent escapings still match.
              </li>
              <li>
                <span className="text-foreground">
                  expires is appended after a literal newline
                </span>{" "}
                so path and timestamp can't be concatenated ambiguously, and
                it's an absolute Unix timestamp — no clock-skew bookkeeping
                between sign and verify.
              </li>
              <li>
                <span className="text-foreground">
                  5-day default, 7-day hard cap.
                </span>{" "}
                <Term>sign()</Term> clamps any requested lifetime into{" "}
                <Term>[1, MAX_EXPIRES_IN]</Term>.
              </li>
              <li>
                <span className="text-foreground">verify() fails closed</span>{" "}
                on a non-finite <Term>expires</Term>, a broken percent-encoding,
                or a length-mismatched token — and uses{" "}
                <Term>timingSafeEqual</Term> for the comparison itself.
              </li>
            </ul>
          </div>
          <div className="flex flex-col gap-4">
            ...
            <CodeBlock
              file="server/src/signer.ts · verify()"
              code={CODE_VERIFY}
            />
